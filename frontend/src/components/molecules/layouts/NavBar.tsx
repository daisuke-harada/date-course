...
export const NavBar: VFC<Props> = memo((props) => {
  ...
  <div className={`${isOpen? `fixed`: `hidden`} lg:hidden sm:top-24 top-20 text-center border border-red-200 bg-red-200 z-50 w-2/3 right-0`}>
    <ul>
      {headerBottomRoutes.map((route) => <li className=" border-b-2 p-2 my-3"><Link className="text-black" to={route.path}>{route.text}</Link></li>)}
      {headerTopLeftRoutes.map((route) =><div className="p-2 my-3"><Link to={route.path} >{route.element}</Link></div>)}
    </ul>
  </div>
  );
});