...
export const CreateCourse: VFC = memo(() => {
  ...
  const getManagementCourse = useRecoilValue(managementCourseState({userId: getCurrentUser.user.id}));
  return(
    <MainDiv>
      <h1 className="m-5 font-bold text-3xl pb-5">デートコース作成</h1>
      <div className="w-full mt-5 flex">
        {
          getManagementCourse.courseDuringSpotIdAndNames.length === 0?
          ...
          <div className="w-1/3">
            {
              getManagementCourse.courseDuringSpotIdAndNames.map((courseDuringSpotIdAndName ,index) => (
                <CourseDuringSpotCard
                  key={courseDuringSpotIdAndName.dateSpotId}
                  addressAndDateSpotId={courseDuringSpotIdAndName.dateSpotId}
                  courseDuringSpotIdAndNames={getManagementCourse.courseDuringSpotIdAndNames}
                  courseNumber={index}
                />
              ))
            }
